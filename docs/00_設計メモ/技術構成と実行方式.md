...
| データベース | PostgreSQL | チャット履歴、ユーザ指示、中間メッセージ、実行状態、回答、参照元、Codex成果物メタ情報を保存する。 |
...
flowchart LR
  ...
  FastAPI --> DB["PostgreSQL<br>履歴・回答"]
...
- PostgreSQLはチャット履歴、ユーザ指示、実行状態、中間メッセージ、回答、参照元、Codex成果物メタ情報を保存する。
...
`validator.max_retries` は、検証失敗後に生成用codex execへ修正を依頼する最大回数である。初回生成は回数に含めない。

上記の例では、初回生成後に検証で失敗した場合、最大2回まで再生成を試みる。2回の再生成後も検証に失敗した場合、画面には回答ではなくエラーメッセージを表示する。

`ui.input_suggestions` は、開始画面に表示する入力候補チップの文字列配列である。各値を1つの候補チップとして表示し、利用者が選択した値を入力欄へ反映する。

`ui.input_suggestions` が未設定の場合は、入力候補チップを表示しない。
...
共有データソースの配置先は `datasource.dir` で指定する。PDF検索アプリを構成する場合の設定例では `codex/readonly/` 配下に配置する。`codex/sessions/` と `codex/sessions_validator/` は、その設定例におけるセッションベースディレクトリである。`codex/saved_artifacts/` は、生成用codex execのセッション内 `artifacts/` からコピーした保存済みCodex成果物本体の保存領域である。
