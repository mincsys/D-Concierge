...
初回実行時のJSONLに含まれる `thread.started` の `thread_id` を指定して、`codex exec resume` で継続指示を実行できることを確認した。
...
- `-C` を `resume` より前に指定することで、継続指示時も `codex/sessions/user-id-xxxxx/id-xxxxx` が作業ディレクトリになった。
...
今回の実測は生成用ホームと生成用作業ディレクトリで行った。`resume`、`--json`、`--output-schema`、`-C` の関係はcodex execの起動方式に依存するため、検証用codex execでも同じ指定順で利用する設計とする。

検証用codex execでは、同一チャットの初回参照元検証で通常の `codex exec` を使い、JSONLの `thread.started.thread_id` を検証用Codex側の会話継続IDとして保存する。2回目以降の参照元検証では、検証用作業ディレクトリを指定して `codex exec resume <validator-codex-thread-id>` を使う。

検証用の継続検証の実行形は次の通りである。

```bash
CODEX_HOME=codex/.codex_validator codex exec --json --output-schema codex/output_json_schema/validator_schema.json -C codex/sessions_validator/<user-id>/<session-id> resume <validator-codex-thread-id> "<参照元検証依頼>"
```

実装前には、`validator.codex.output_schema` で指定するスキーマと `validator.codex.home` でも同じ起動方式が利用できることを確認する。
...
- 生成用codex execは、常に `--json` と `--output-schema` を併用する。
