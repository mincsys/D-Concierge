...
- APIキー、環境変数全文、秘密情報、OS依存の絶対パス、生JSONL全文、コマンド出力全文を保存しない。
...
| --- | --- | --- | --- | --- | --- |
| `write` | トレースログを1件追記する | trace_id、発生段階、エラー分類、関連ID、例外情報、Codex終了情報、検証失敗情報、調査用要約 | なし | トレースログ保存先が設定済みであること | JSONLとして1行追記されること<br>保存禁止情報がマスクされること |
