...
| --- | --- | --- | --- | --- | --- |
| `run_generation` | 生成用codex execを起動する | ユーザ指示、生成用設定、作業領域、会話継続ID、trace_id | 生成結果、JSONLイベント | 生成用ホームと作業領域が利用可能であること | 完了時は回答候補と生成用Codex側の会話継続IDが返ること |
| `run_validation` | 検証用codex execを起動する | 検証指示、検証用設定、作業領域、会話継続ID、trace_id | 検証結果、JSONLイベント | 検証用ホームと作業領域が利用可能であること | 完了時は検証結果と検証用Codex側の会話継続IDが返ること |
...
| `process_completed` | codex execが正常終了したとき | 終了コード、会話継続ID、最終候補を通知する |
