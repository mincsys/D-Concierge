...
| バックエンド application port | Codex実行IF | `application/execution`、`application/validation`、`application/chat` | `application/ports/codex/interface.py`、実装は `infrastructure/codex` | 生成用Codex実行、参照元検証、終了制御、作業領域解決、作業領域削除を抽象化する。 | [Codex実行IF.md](Codex実行IF.md) |
...
| バックエンド application port | 成果物ファイルIF | `application/artifacts`、`application/chat` | `application/ports/filesystem/interface.py`、実装は `infrastructure/filesystem/artifacts` | 採用済みCodex成果物の保存、配信用読込、保存済み成果物実体削除を抽象化する。 | [成果物ファイルIF.md](成果物ファイルIF.md) |
