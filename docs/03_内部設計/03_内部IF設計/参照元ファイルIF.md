...
| ページ範囲不正 | 保存済み回答の参照元取得ではDB上のlocatorを信頼する。回答候補検証ではCodex検証実装側の起動前確認で扱う |

## 8. 留意事項

- PDF存在確認とページ数の厳密な検証は、回答候補の参照元検証で検証用codex execを起動する前に、Codex検証実装側の固定検証として実施する。本IFは保存済み参照元の配信用読込を扱う。
