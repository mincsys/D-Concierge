...
- ホスト側の `datasource` を `/workspace/readonly` に読み取り専用でマウントする。
...
- `/workspace` には `readonly` と `artifacts` が見えた。
...
Codexが起動したシェルコマンドから、`/workspace/readonly` と `/workspace/artifacts` への書き込み可否を確認した。

結果:

- `/workspace/readonly` には書き込めなかった。
...
| `datasource_dir` | `/workspace/readonly` |
...
- `datasource_dir` はコンテナ内 `/workspace/readonly` へ読み取り専用マウントする。
...
この方式により、Codex実行時に見えるファイルシステムを `/workspace/readonly` と `/workspace/artifacts` に限定しやすくなる。現行の `codex exec --json`、`--output-schema`、`resume` を維持できるため、`codex app-server` へ切り替えるよりも現在の実装への影響は小さい。
