...
D-Conciergeは、利用者が自然文で質問すると、内部でCodex execがデータソースを検索・分析し、参照元を明示して回答する汎用的な質問応答・分析チャットアプリである。
...
D-Concierge本体は、PDF検索、説明書検索、センサデータ分析、コードベース調査、設計書・議事録検索などに転用できる共通基盤として定義する。PDF検索は、用途別アプリを構成する場合の例である。
...
- Codex execの最終出力を設定で指定した出力スキーマに従うJSONとして受け取れる。
...
- PDF参照元ビューアを設定した場合は、参照元リンクから該当PDFページをポップアップ表示できる。
...
MVPではログインを必須にしない。ただし、バックエンドはDB上にローカル利用者IDを保持し、チャット履歴とCodex execのセッションディレクトリを同じ利用者IDに紐づける。

例:

- PDF検索用途では、利用者はIPA資料やSEC BOOKSの内容について質問する。
...
| 参照元 | 回答が参照した情報ソースと位置情報。PDFページ、コード行範囲などを含む。 |
...
flowchart LR
  ...
  GenCodex --> DataSource["共有データソース<br>codex/readonly 等"]
  GenCodex --> Backend
...
| アプリ種別 | 主なデータソース | 生成用Skillsの例 | 参照元ビューア・表示の例 |
| --- | --- | --- | --- |
| PDF検索 | PDF、HTML化済み文書、メタJSON | 構造化HTML検索、キーワード検索 | PDFページポップアップ |
| 説明書検索 | 製品マニュアル、FAQ、HTML化済み説明書 | 章節検索、エラーコード検索 | 文書ページ表示 |
| センサ分析 | CSV、Parquet、時系列DB抽出ファイル | 時系列集計、異常検知、比較分析 | 回答内画像・グラフ表示 |
| コード調査 | ソースコード、設計書、議事録 | grep検索、AST検索、設計書検索 | コード行範囲表示 |

### 7.2 設定ファイル

アプリケーションは、設定ファイルでCodex execの実行場所、出力スキーマ、検証回数、API、データベースなどの実行時設定を定義する。
...
Codex execの実行場所は、アプリケーション設定で指定する。`codex.home` は生成用Codex execが読み込む `AGENTS.md`、Skills、Codex設定の配置先であり、このディレクトリには生成用の `AGENTS.md` とSkillsが含まれる前提とする。`codex.workdir` は生成用セッションディレクトリを作成するベースディレクトリである。

検証用Codex execの実行場所は、`validator.codex.home` と `validator.codex.workdir` で指定する。`validator.codex.home` は検証用Codex execが読み込む `AGENTS.md`、Skills、Codex設定の配置先であり、このディレクトリには検証用の `AGENTS.md` とSkillsが含まれる前提とする。`validator.codex.workdir` は検証用セッションディレクトリを作成するベースディレクトリである。

生成用・検証用Codex execの振る舞いは、それぞれ設定された `AGENTS.md` とSkillsに従って制御する。

設定値が相対パスの場合は、アプリケーション実行基準ディレクトリからの相対パスとして扱う。

バックエンドは同じ利用者IDとセッションIDを使って、生成用には `codex.workdir/<user-id>/<session-id>/`、検証用には `validator.codex.workdir/<user-id>/<session-id>/` を作成する。上記のPDF検索アプリを構成する場合の設定例では、生成用は `codex/sessions/<user-id>/<session-id>/`、検証用は `codex/sessions_validator/<user-id>/<session-id>/` になる。
...
Codex資産と共有データソースの配置は、アプリケーション設定で指定する。MVPの共通基盤は具体的なディレクトリ名に依存しない。

設定項目と役割は次の通りである。

- `codex.home`: 生成用 `AGENTS.md`、生成用Skills、生成用Codex設定の配置先。
- `codex.workdir`: 生成用セッションベースディレクトリ。
- `codex.output_schema`: 生成用Codex execの出力スキーマ。
- `validator.codex.home`: 検証用 `AGENTS.md`、検証用Skills、検証用Codex設定の配置先。
...
- 回答表示領域
  ...
  - 設定または入力補助ボタン
- 参照元表示領域
  - PDF参照元ビューアを設定した場合は、参照元クリック時にPDFポップアップとして表示する。
...
バックエンドは、新規チャットまたは新規チャット実行時に、DB上のローカル利用者IDとセッションIDを使って新しいセッションディレクトリを作成する。過去チャットを継続する場合は、同じ利用者IDとセッションIDに対応する既存のセッションディレクトリを再利用し、Codex execが前回までの中間作業や生成物を同じ文脈として扱えるようにする。
...
実行時の作業ディレクトリは、`codex.workdir/<user-id>/<session-id>` として決定する。生成用Codex execの `CODEX_HOME` には `codex.home` を指定し、`-C` にはこの作業ディレクトリを指定する。

PDF検索アプリを構成する場合の設定例では、次のように起動する。
...
`<user-id>` はDB上の利用者ID、`<session-id>` はチャット実行または継続チャットを識別するセッションIDである。バックエンドは、アプリケーション設定により、用途に対応する `AGENTS.md` とSkillsを読み込ませる。

生成用セッションディレクトリは、次の構成を持つ。下記はPDF検索アプリを構成する場合の設定例である。

```text
codex/sessions/<user-id>/<session-id>/
  readonly -> ../../../readonly
  tmp/
  artifacts/
```

`readonly/` は共有データソースへのシンボリックリンクであり、PDF、HTML化済み文書、メタデータなどを参照する。`tmp/` はCodex execの中間作業を保持する。`artifacts/` はCodex execが生成した画像、HTML、CSV、その他の出力ファイルの一時出力先であり、回答Markdownから参照される生成物を置く。
...
バックエンドは、固定検証を通過した回答JSONに対して、検証用Codex execを非対話実行する。

実行時の作業ディレクトリは、`validator.codex.workdir/<user-id>/<session-id>` として決定する。検証用Codex execの `CODEX_HOME` には `validator.codex.home` を指定し、`-C` にはこの作業ディレクトリを指定する。
...
検証用セッションの `<user-id>` と `<session-id>` は、生成用セッションと同じ値を使う。

検証用セッションディレクトリは、次の最小構成を持つ。下記はPDF検索アプリを構成する場合の設定例である。

```text
codex/sessions_validator/<user-id>/<session-id>/
  readonly -> ../../../readonly
  tmp/
```

検証用Codex execは、回答の改善ではなく、回答JSONの参照元が妥当かを判定するために実行する。検証用セッションには `artifacts/` を作成しない。
...
MVPではログインを前提にしないが、バックエンドはDB上のローカル利用者IDを発行・保持し、履歴はローカル利用者単位で保存する。Codex execの生成用・検証用セッションディレクトリも同じ利用者IDを使って分離する。
...
- セッションディレクトリ内の `readonly/` シンボリックリンクを扱うパス検証は、`realpath` ベースで行う。
- `readonly/` の実体は、許可された共有データソース配下に限定する。
- `tmp/` と `artifacts/` は、対象セッションディレクトリ配下から出ていないことを検証する。
- `readonly/` 配下は、生成物取り込み対象にしない。
- HTMLは必ずサニタイズする。
- 任意JavaScriptを実行しない。
...
- アプリケーション設定、`AGENTS.md`、Skills、Codex execのセッションベースディレクトリ、共有データソースはアプリケーション単位で差し替えられる。
- `readonly/` は共有データソースなので、セッションごとに複製しない。
- 履歴表示では、DBに保存したメッセージとDBに保存したartifactを使う。
- `artifacts/` 内のファイルは、セッション継続時にCodex execが後から書き換える可能性があるため、画面表示や履歴表示では直接参照しない。
...
- Mermaid図を表示できる。
- サニタイズ済みHTMLを表示できる。
- 設定された `source_type` に対応する参照元ビューアを開ける。
- PDF参照元ビューアを設定した場合は、参照元リンクから該当PDFページをポップアップ表示できる。
- 過去チャット履歴を一覧表示できる。
- チャット履歴を検索できる。
- 出力スキーマ外の `source_type` を固定検証で拒否できる。
