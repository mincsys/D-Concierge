...
D-Conciergeは、利用者が自然文で質問すると、内部でCodex execがデータソースを検索・分析し、根拠付きで回答する質問応答・分析チャットアプリである。
...
今回の代表アプリは、PDF書籍データを検索し、回答根拠として該当PDFページをポップアップ表示するチャットアプリである。ただし、MVPの要件はPDF書籍専用ではなく、説明書検索、センサデータ分析、コードベース調査、設計書・議事録検索などにも転用できる共通基盤として定義する。
...
- 回答表示前に、固定検証とCodex execによる根拠検証を実行できる。
- 検証に成功した回答だけをチャット画面に表示できる。
- 回答本文はMarkdownを基本として表示し、画像、Mermaid図、サニタイズ済みHTMLも表示できる。
- 回答中の根拠リンクから、データソース種別に応じたビューアを開ける。
- PDF書籍アプリでは、根拠リンクから該当PDFページをポップアップ表示できる。
...
利用者は、チャット画面から質問を入力し、回答と根拠を確認する人である。
...
| 検証用Codex exec | 生成された回答JSONの根拠が正しいかを検証するCodex exec。 |
| 固定検証 | アプリケーション側の決定的な検証処理。JSON構造、必須項目、対応済み根拠種別などを確認する。 |
| 根拠 | 回答の根拠となった情報ソースと位置情報。PDFページ、コード行範囲、時系列データ範囲などを含む。 |
| 根拠ビューア | 根拠種別に応じて情報ソースを表示する画面部品。 |
...
flowchart LR
  ...
  FixedValidation --> ValidateCodex["検証用Codex exec<br>根拠確認"]
  ValidateCodex --> DataSource
  ValidateCodex --> Backend
  Backend --> Frontend
  Frontend --> EvidenceViewer["根拠ビューア<br>PDFポップアップ等"]
```

バックエンドは、利用者の質問を受け取ると生成用Codex execを起動する。生成用Codex execは、アプリケーションに同梱された `AGENTS.md` とSkillsに従い、作業ディレクトリ内のデータソースを検索・分析する。

生成された回答JSONは、そのまま画面に表示しない。バックエンドは固定検証を行い、さらに検証用Codex execで根拠の正しさを確認する。検証に通過した回答だけをフロントエンドへ返す。
...
| 領域 | 採用技術 | 用途 |
| --- | --- | --- |
| フロントエンド | React + TypeScript + Vite | チャット画面、履歴画面、根拠ビューアを構築する。 |
...
| PDF表示 | PDF.js | 根拠PDFの該当ページをポップアップ表示する。 |
...
| アプリ種別 | 主なデータソース | 生成用Skillsの例 | 根拠ビューアの例 |
| --- | --- | --- | --- |
...
アプリケーションは、設定ファイルでCodex execの実行場所、スキーマ、検証回数、対応する根拠種別を定義する。
...
- メイン領域
  ...
  - 設定または入力補助ボタン
- 根拠表示領域
  - PDF書籍検索アプリでは、根拠クリック時にポップアップとして表示する。
...
- 必要に応じて分析、比較、要約を行う。
- 回答本文と根拠を含むJSONを出力する。
- 出力直前に、根拠ページや根拠箇所が本当に回答を支えているか確認する。
...
共通拡張スキーマは、データソースの種類に依存しない項目と、根拠種別ごとの位置情報を分けて表現する。
...
- 回答は複数のブロックに分割できる。
- 各回答ブロックは、0件以上の根拠を持つ。
- 根拠は `source_type` で種類を示す。
- 根拠の具体的な位置情報は `locator` に格納する。
...
```json
{
  ...
  "excerpt": "目的や背景を共有し、関係者間で合意形成することが重要である。"
}
```
...
```json
{
  ...
  "excerpt": "セッションの有効期限は設定値から読み込まれる。"
}
```
...
1. 固定検証
2. 検証用Codex execによる根拠検証

固定検証は、アプリケーション側で決定的に判定できる内容を確認する。検証用Codex execは、根拠情報が回答内容を本当に支えているかを確認する。
...
```json
{
  ...
  "path": "$.answers[0].evidence[1].source_type"
}
```
...
根拠をPDFページ形式に修正して、スキーマに適合するJSONを再出力してください。
```

### 11.4 検証用Codex execによる根拠検証

検証用Codex execは、生成用Codex execとは別の `AGENTS.md` とSkillsを使う。

検証用 `AGENTS.md` は、回答を改善するのではなく、回答JSONの根拠が妥当かを判定するための指示を持つ。

検証観点:

- 回答本文の主張が、提示された根拠に実際に記載されているか。
- 根拠ページや根拠範囲がずれていないか。
- 回答の一部に根拠が不足していないか。
- 根拠の抜粋が、回答内容と矛盾していないか。
- 複数の根拠を組み合わせた推論が過剰でないか。
  ...
  "findings": [
    {
      ...
      "message": "回答では優先順位付けについて述べているが、根拠PDF p.42-45には該当する記述が確認できません。"
    }
  ]
}
```
...
PDF書籍検索アプリでは、`pdf_page` 根拠をクリックすると、該当PDFの該当ページをポップアップで表示する。
...
- PDFの読み込みに失敗した場合は、根拠ラベルとエラーメッセージを表示する。
...
リンクのうち、根拠リンクは通常の外部リンクではなく、回答JSONの根拠情報に紐づく内部操作として扱う。
...
| 固定検証エラー | スキーマ不一致、未対応根拠種別など | 再生成し、上限超過時はエラー表示 |
| 根拠検証エラー | 根拠が回答を支えていない | 再生成し、上限超過時はエラー表示 |
| ビューア表示エラー | PDFや根拠ファイルを開けない | 根拠表示に失敗したことを表示 |
...
- 検証用Codex execで根拠検証を実行できる。
- 検証失敗時、設定上限まで生成用Codex execへ修正依頼できる。
- 検証成功後にだけ回答を表示できる。
- Markdown回答を表示できる。
- Mermaid図を表示できる。
- サニタイズ済みHTMLを表示できる。
- PDF書籍検索アプリで、根拠リンクから該当PDFページをポップアップ表示できる。
- 過去チャット履歴を一覧表示できる。
- チャット履歴を検索できる。
