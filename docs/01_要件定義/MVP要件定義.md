...
```bash
codex exec --json --output-schema codex/output_json_schema/pdf-reference-schema.json -C codex/work "<利用者の質問>"
```
...
MVPのPDF書籍検索アプリでは、`source_type` は `pdf` のみを対応必須とする。コード調査アプリなど別用途のアプリでは、`codex.output_schema` を差し替えて `source_type` と `locator` の形を変える。
...
PDF書籍検索アプリでは、`pdf` 参照元をクリックすると、該当PDFの該当ページをポップアップで表示する。
