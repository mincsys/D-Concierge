...
本書は、D-ConciergeバックエンドとCodex実行コンテナの間で利用する起動、入力、出力、キャンセル、異常時の扱いを定義することを目的とする。

## 2. 前提

- 生成用Codex実行と検証用Codex実行は別設定で実行する。
- Codexは実行ごとにDockerコンテナ内で起動し、実行終了後にコンテナを削除する。
- コンテナ内では標準出力JSONLを逐次読み取れる方式で `codex exec` を実行する。
- それぞれのCodexホームディレクトリには `AGENTS.md` とSkillsが含まれる。
- 作業ディレクトリはDBに保存されたユーザIDとセッションIDで分離する。
- 生成用Codex実行から得た未検証回答は、検証成功まで最終回答として扱わない。
- 最終回答または検証結果は、設定された出力契約に適合する構造化データとして受け取る。
- 継続指示では、初回実行時に取得した生成用Codex側の会話継続IDを指定して生成用Codex実行を再開する。
- 検証用Codex実行は、初回検証時に取得した検証用Codex側の会話継続IDを指定して2回目以降の検証を再開する。
...
| --- | --- |
| 送信元または起動元 | バックエンド |
| 受信先または処理先 | 生成用Codex実行コンテナ、検証用Codex実行コンテナ |
| 方向 | 双方向 |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| IF-CX-01 | 生成用Codex実行 | 回答候補を生成する。 | ユーザ指示送信受付後、継続指示受付後、または再生成時 | 送信 / 受信 | Dockerコンテナ実行 | 回答生成、トレースログ保存 | 継続指示では生成用Codex側の会話継続IDを指定して再開する。 |
| IF-CX-02 | 検証用Codex実行 | 生成用Codexの回答を検証する。 | 固定検証処理成功後 | 送信 / 受信 | Dockerコンテナ実行 | 回答検証、トレースログ保存 | 2回目以降は検証用Codex側の会話継続IDを指定して再開する。 |
| IF-CX-03 | Codex実行キャンセル | 実行中のCodex実行コンテナを終了させる。 | 利用者のキャンセル要求時 | 送信 / 受信 | Dockerコンテナ制御 | キャンセル、トレースログ保存 | OS差異はバックエンド内部で吸収する。 |

## 5. IF詳細

### 5.1. IF-CX-01 生成用Codex実行
...
| 実施条件 | 生成用ホームディレクトリ、生成用作業領域、共有データソース、出力契約を利用できること。継続指示時は生成用Codex側の会話継続IDを利用できること。 |
...
| 出力契約 | 必須 | 構造化データ | 最終回答が満たすべき構造を示す。Codex実行が受理できる形式であること。 | `generator.output_schema` から導出し、起動時に出力スキーマとして指定する。 |
...
| 回答候補 | 必須 | 構造化データ | 出力契約に適合すること。検証成功まで最終回答として扱わない。 | 最後のエージェントメッセージを完了後に固定検証処理へ渡す。 |
...
| 後続処理 | 生成用Codex側の会話継続IDをチャットに紐づけて保存する。回答候補とCodex成果物候補を固定検証処理へ渡す。 |

#### 5.1.5. 異常時の扱い

| 異常事象 | 検知方法 | システムの扱い | 業務上の扱い | 再実行方針 |
| --- | --- | --- | --- | --- |
| 起動失敗 | Codex実行コンテナまたはコンテナ内Codexプロセスを開始できない。 | チャット実行処理の状態をエラーにし、トレースログを保存する。 | 回答を表示できない。 | 利用者による同一ユーザ指示の再送で再実行する。 |
| マウント失敗 | ホームディレクトリ、作業領域、共有データソース、成果物領域をマウントできない。 | チャット実行処理の状態をエラーにし、トレースログを保存する。 | 回答を表示できない。 | 設定またはファイル配置の修正後に再実行する。 |
| タイムアウト | 設定された時間内に完了しない。 | チャット実行処理の状態をタイムアウトにし、トレースログを保存する。 | タイムアウトを表示する。 | 利用者が必要に応じて再送する。 |
| 出力契約不正 | Codex実行が出力契約を受理せず、起動後にエラーを返す。 | チャット実行処理の状態をエラーにし、トレースログを保存する。 | 回答を表示できない。 | 設定修正後に再実行する。 |
...
| プロセス異常終了 | JSONLエラーを受信せずにCodex実行が非ゼロ終了する。 | チャット実行処理の状態をエラーにし、終了コードと標準エラー要約をトレースログに保存する。 | 予期しないエラーを表示する。 | 運用者確認後に再実行する。 |
...
| 実施条件 | 回答候補、参照元情報、検証指示、検証用作業領域を利用できること。2回目以降のCodex検証処理では検証用Codex側の会話継続IDを利用できること。 |

#### 5.2.2. 入力情報

| 項目名 | 必須 | 型・形式 | 制約 | 備考 |
| --- | --- | --- | --- | --- |
| 回答候補 | 必須 | 構造化データ | 固定検証処理に成功していること。 | 生成用Codex実行の最終回答候補をJSONとしてパースしたもの。 |
| 参照元情報 | 必須 | 構造化データ | 回答候補の根拠として検証対象にする。 |  |
| 検証指示 | 必須 | 文字列または構造化データ | Codex検証処理の観点を示す。 | 検証用ホームディレクトリの指示に従う。 |
...
| 検証結果 | 必須 | 構造化データ | 生成用Codexの回答を検証した結果を示し、`payload.kind="final"`、`valid`、`comment` を満たす。 | 最後のエージェントメッセージを完了後に採用候補にし、バックエンドで固定検証処理を行う。 |
| 中間メッセージ候補 | 任意 | 構造化データ | `item.completed` の `agent_message.text` が `payload.kind="progress"` のJSONである場合だけ対象にする。 | `payload.text` を画面へ配信する。`payload.kind="final"`、内部パス、コマンド、標準出力は配信しない。 |
...
| Codex JSONLエラー | JSONLで `type:error` または `turn.failed` を受信する。 | チャット実行処理の状態をエラーにし、Codexのエラーメッセージをトレースログに保存する。 | AIサービスプロバイダ側エラーを表示する。 | 利用者が必要に応じて再送する。 |
| 検証処理失敗 | 検証用Codex実行の起動失敗、マウント失敗、または異常終了を検知する。 | チャット実行処理の状態をエラーにし、トレースログへ記録する。 | 回答を表示できないことを示す。 | 利用者が必要に応じて再送する。 |
...
| 実施条件 | 対象チャット実行処理に紐づくCodex実行コンテナが実行中であること。 |
...
| 対象コンテナ情報 | 必須 | 構造化データ | OS差異をバックエンド内部で吸収できる情報を含む。 | 利用者へは表示しない。 |
...
| 項目 | 内容 |
| --- | --- |
| 正常終了条件 | 対象Codex実行コンテナを終了できること。 |
| 結果通知 | バックエンドへキャンセル結果を返す。 |
| 後続処理 | チャット実行処理の状態をキャンセル済みにし、部分回答や途中Codex成果物を最終回答にしない。終了要求後もコンテナが残る場合は削除対象として扱う。 |

#### 5.3.5. 異常時の扱い

| 異常事象 | 検知方法 | システムの扱い | 業務上の扱い | 再実行方針 |
| --- | --- | --- | --- | --- |
| キャンセル失敗 | 対象コンテナを終了できない。 | トレースログへ保存し、対象コンテナを削除対象として扱い、利用者向けエラーを表示する。 | 利用者はキャンセル完了を確認できない。 | 状態再取得後に必要なら再要求する。 |
| 対象コンテナなし | 対象チャット実行処理に紐づく実行中コンテナを確認できない。 | 現在状態を確認し、完了済みならキャンセル不可として扱う。 | 利用者は最新状態を確認する。 | 履歴詳細を再取得する。 |
...
| 生成用Codex側の会話継続ID | 任意 | 識別子 | 生成用Codex実行の継続指示で使う。 | D-ConciergeのセッションIDとは別に管理する。 |
| 検証用Codex側の会話継続ID | 任意 | 識別子 | 検証用Codex実行の2回目以降の検証で使う。 | 生成用Codex側の会話継続IDとは別に管理する。 |
| ホームディレクトリ | 必須 | パス | `AGENTS.md` とSkillsを含む。 | ホスト側ディレクトリをコンテナ側 `codex_docker.codex_home_dir` へマウントする。 |
| 作業ディレクトリ | 必須 | パス | ユーザIDとセッションIDで分離する。 | ホスト側ディレクトリをコンテナ側 `codex_docker.workspace_dir` へマウントする。 |
| 共有データソース | 必須 | パス | コンテナ内から読み取り専用で参照できること。 | セッションごとに複製しない。 |
| 成果物領域 | 任意 | パス | 生成用は書き込み可能、検証用は存在時に読み取り可能でマウントする。 | 生成用と検証用で見える内容をできるだけ一致させる。 |
| 出力スキーマ | 必須 | 構造化データ | Codex実行が受理できるJSON Schemaであること。 | 生成用と検証用で別内容にできる。 |
...
- Codex実行コンテナはCodex本体のAIサービス通信を行える状態で起動する。
- Codexが実行するシェルコマンドのネットワーク可否はCodex設定で制御する。
- Codex認証情報は設定ファイルから渡し、利用者向けAPI、SSE、画面表示に含めない。
...
| 設定不足 | ホームディレクトリ、作業ディレクトリ、指示ファイル、Docker実行設定を確認できない。 | 対象チャット実行処理をエラーにする。 | 回答生成または検証を開始できない。 | 設定修正後に再実行する。 |
| JSONL解析失敗 | 標準出力のイベントを構造化データとして解釈できない。 | 対象チャット実行処理をエラーにし、トレースログを保存する。 | 回答または検証結果を表示できない。 | 利用者が必要に応じて再送する。 |

## 7. 運用上の留意事項

- 起動時のDockerイメージ、コンテナ側作業ディレクトリ、コンテナ側Codexホーム、認証情報、ホスト側のホームディレクトリ、作業ディレクトリは設定ファイルから導出する。
- 起動時は標準出力JSONLと出力スキーマを利用し、最終結果もJSONLから取得する。
- 継続指示時は、同じ生成用作業ディレクトリを指定し、生成用Codex側の会話継続IDで生成用Codex実行を再開する。
- 2回目以降の検証時は、同じ検証用作業ディレクトリを指定し、検証用Codex側の会話継続IDで検証用Codex実行を再開する。
...
- 共有データソースはセッションごとに複製せず、Codex実行コンテナへ読み取り専用でマウントする。
- 生成用Codexの成果物領域が存在する場合は、検証用Codex実行コンテナにも同じ領域をマウントする。
- 中間メッセージ候補やJSONLイベントには内部パス、秘密情報、環境変数を表示しない。
- コマンド実行時の通知に含まれるコマンド文字列、標準出力、絶対パスは利用者画面へ直接表示しない。
- Windows/Linuxの具体的なDocker制御差異はバックエンド内部に閉じ込め、キャンセル時は対象コンテナへ終了要求を送り、残存する場合は削除対象として扱う。
