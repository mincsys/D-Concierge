...
| 中間メッセージ | 任意 | SSEイベント | 生成または検証の途中状況を配信する。 | event: `message`。payloadは中間メッセージイベント情報とする。 |
| 最終回答 | 任意 | SSEイベント | 検証済み回答本文と表示用参照元メタ情報を配信する。 | event: `answer`。payloadは最終回答イベント情報とする。 |
...
| 結果通知 | SSE接続成立直後に現在状態を配信し、以後は状態変化、中間メッセージ、最終結果を配信する。中間メッセージは画面表示用に整形・マスク済みの本文だけを履歴再表示用に保存する。 |
...
| 回答本文 | 必須 | 文字列 | 検証済み回答本文であること。 | 回答表示情報のAPI項目名: `markdown` |
| 参照元 | 任意 | 表示用参照元メタ情報配列 | 回答に参照元がある場合に返す。 | 回答表示情報のAPI項目名: `references` |

回答本文にCodex成果物を埋め込む場合は、`/api/artifacts/{artifact_id}` 形式のURLを使用する。
