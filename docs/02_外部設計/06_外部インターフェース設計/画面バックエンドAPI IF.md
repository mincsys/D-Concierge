...
- 利用者操作によって表示対象を切り替える場合、画面は現在のSSE購読を解除し、切断エラーとして扱わない。
...
| 補足案内文 | 任意 | 文字列 | 未設定時は返さない、または空扱いとする。 | API項目名: `sub_welcome_message` |
...
| 後続処理 | 開始画面のウェルカムメッセージ、補足案内文、入力候補チップへ反映する。 |

#### 5.1.5. 異常時の扱い

| 異常事象 | 検知方法 | システムの扱い | 業務上の扱い | 再実行方針 |
| --- | --- | --- | --- | --- |
| 設定取得失敗 | バックエンド処理で設定を取得できない。 | 利用者向けエラーを返さず、設定なし相当で応答する。 | 画面は利用できるが、ウェルカムメッセージ、補足案内文、入力候補チップを表示しない。 | 画面再読み込み時に再取得する。 |
...
| 受付状態 | 必須 | 列挙値 | `accepted` を返す。 | API項目名: `state` |
...
| 後続処理 | 画面は実行状態と回答表示を更新し、終了状態受信後に履歴一覧を再取得する。接続成立直後の状態通知が終了状態の場合は、履歴詳細と履歴一覧を再取得する。表示対象を切り替える場合は現在のSSE購読を解除する。 |
...
| 項目名 | 必須 | 型・形式 | 制約 | 備考 |
| --- | --- | --- | --- | --- |
| キャンセル処理状態 | 必須 | 列挙値 | 変更前状態が `accepted` の場合は `canceled`、`running` または `validating` の場合は `cancel_requested` を返す。 | API項目名: `state` |
| 利用者向けメッセージ | 必須 | 文字列 | `state` が `canceled` の場合は `処理をキャンセルしました。`、`cancel_requested` の場合は `処理をキャンセルしています。` を返す。 | API項目名: `user_message` |
...
| 結果通知 | REST応答でキャンセル処理状態と利用者向けメッセージを返し、SSEで状態変更およびキャンセル済みを配信する。 |
...
| チャット状態 | 必須 | 列挙値 | `deleting` を返す。 | API項目名: `chat_state` |
...
| 項目名 | 必須 | 型・形式 | 制約 | 備考 |
| --- | --- | --- | --- | --- |
| ログインセッションCookie | 任意 | Cookie | Cookieがある場合は現在のログインセッションとして扱う。 | Cookieがない場合もログアウト済み確認として実行できる。リクエストボディは使用しない。 |
...
| ログインセッションCookie | 必須 | Cookie | ブラウザ側Cookieを失効させる。 | Cookieに対応するセッション行がある場合はサーバ側で削除する。 |

#### 5.14.4. 正常時の扱い

| 項目 | 内容 |
| --- | --- |
| 正常終了条件 | 現在のCookieに対応するログインセッション行が存在しない状態になること。 |
...
| 最終更新日時 | 必須 | 日時文字列 | 履歴一覧の並び順に利用する。 | API項目名: `updated_at` |
| チャット状態 | 必須 | 列挙値 | `active`、`deleting` のいずれか。 | API項目名: `chat_state`。削除受付結果で使用し、実行処理の状態とは別に扱う。 |
...
| 実行状態 | 必須 | 列挙値 | `accepted`、`running`、`validating`、`cancel_requested`、`canceled`、`completed`、`error`、`timed_out` のいずれか。 | API項目名: `state` |
...
| 本文 | 配信タイミング |
| --- | --- |
| `作業を開始します。` | ユーザ指示を受け付けた後、初回の生成用Codex実行を起動する前。再生成時は配信しない。 |
| `作業が完了しました。` | 生成用Codex実行が最終回答候補を返した直後。 |
| `回答の検証を開始します。` | 回答候補の検証処理を開始する直前。 |
| `回答の検証を完了しました。` | 回答候補の検証が合格した直後。 |
| `回答を修正します。` | 検証が不合格で再生成可能な場合に、次の生成用Codex実行を起動する前。 |
