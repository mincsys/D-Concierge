...
| 実施条件 | 対象チャットに実行中または検証中のチャット実行処理がないこと。 |
...
| 実行中の継続指示 | 対象チャットに実行中または検証中のチャット実行処理がある。 | 受付不可のエラーを返す。 | 現在の回答完了後に送信できる。 | 完了後に利用者が再送する。 |
...
| 最終回答 | 任意 | SSEイベント | 検証済み回答だけを配信する。 | event: `answer` |
...
| 項目名 | 必須 | 型・形式 | 制約 | 備考 |
| --- | --- | --- | --- | --- |
| 参照元ID | 必須 | UUID文字列 | 保存済み参照元を指定する。 | API項目名: `reference_id` |
...
| 後続処理 | 画面は参照元種別に対応する参照元ビューアへ表示する。 |
...
| 参照元ID | 必須 | UUID文字列 | 参照元を一意に識別する。 | API項目名: `reference_id` |
