...
| 回答候補 | 必須 | 構造化データ | 出力契約に適合すること。PDF参照元pathは `readonly/` から始まる実PDFファイルへの相対パスであること。検証成功まで最終回答として扱わない。 | `readonly/html/<文書名>/index.html` は検索・本文確認用であり、回答候補のPDF参照元には対応する `readonly/raw/pdf/<文書名>.pdf` を指定する。最後のエージェントメッセージを完了後に形式検証へ渡す。 |
