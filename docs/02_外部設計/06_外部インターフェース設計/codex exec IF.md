...
| 方向 | 双方向 |
| 主要情報 | ユーザ指示本文、出力契約、生成用Codex側の会話継続ID、検証用Codex側の会話継続ID、JSONLイベント、回答候補、参照元、検証結果、中間メッセージ、Codex成果物 |
...
| 出力契約 | 必須 | 構造化データ | 最終回答が満たすべき構造を示す。codex execが受理できる形式であること。 | `codex.output_schema` から導出し、起動時に出力スキーマとして指定する。 |
| 生成用Codex側の会話継続ID | 継続指示時必須 | 識別子 | 新規チャットでは指定しない。継続指示では初回実行時に取得した値を指定する。 | JSONLの `thread.started` で得た `thread_id`。 |
| セッション作業領域 | 必須 | パス | 利用者IDとセッションIDで分離する。 | OS依存表現は利用者へ表示しない。 |
...
| 生成用Codex側の会話継続ID | 必須 | 識別子 | 新規チャット初回実行後は、以後の継続指示で利用できるよう保存する。 | JSONLの `thread.started` で得た `thread_id`。 |
| 中間メッセージ候補 | 任意 | 構造化データ | 最終回答ではないと判定できたエージェントメッセージだけを対象にする。 | 表示本文を安全に抽出できる場合だけ画面へ配信する。 |
| 回答候補 | 必須 | 構造化データ | 出力契約に適合すること。検証成功まで最終回答として扱わない。 | 最後のエージェントメッセージを完了後に形式検証へ渡す。 |
| Codex成果物 | 任意 | ファイルまたは参照情報 | 履歴表示では保存済みCodex成果物を使う。 | バックエンドが保存済みCodex成果物領域へコピーしてメタ情報を保存する。 |
...
| 後続処理 | 生成用Codex側の会話継続IDをチャットに紐づけて保存する。回答候補を形式検証へ渡し、Codex成果物は保存済みCodex成果物領域へコピーしてメタ情報を保存する。 |
...
| 検証用Codex側の会話継続ID | 2回目以降必須 | 識別子 | 同一チャットの初回検証では指定しない。2回目以降の検証では初回検証時に取得した値を指定する。 | JSONLの `thread.started` で得た `thread_id`。 |
| 検証結果出力契約 | 必須 | 構造化データ | 検証結果が満たすべき構造を示す。codex execが受理できる形式であること。 | `validator.codex.output_schema` から導出し、起動時に出力スキーマとして指定する。 |
| 検証用ホームディレクトリ | 必須 | パス | `AGENTS.md` とSkillsを含む。 | 設定ファイルから導出する。 |
| 検証用作業領域 | 必須 | パス | 利用者IDとセッションIDで分離する。 | OS依存表現は利用者へ表示しない。 |
...
| 検証用Codex側の会話継続ID | 必須 | 識別子 | 同一チャットの2回目以降の検証で利用できるよう保存する。 | JSONLの `thread.started` で得た `thread_id`。 |
| 検証結果 | 必須 | 構造化データ | 参照元が回答内容を支えているかを示し、検証結果出力契約に適合する。 | 最後のエージェントメッセージを完了後に採用候補にする。 |
| 中間メッセージ候補 | 任意 | 構造化データ | 最終検証結果ではないと判定できたエージェントメッセージだけを対象にする。 | 表示本文を安全に抽出できる場合だけ画面へ配信する。 |

#### 5.2.4. 正常時の扱い

| 項目 | 内容 |
| --- | --- |
| 正常終了条件 | 検証用Codex側の会話継続IDを取得し、完了イベントを受信し、最後のエージェントメッセージから検証結果を取得できること。 |
...
| 検証処理失敗 | 検証用codex execの起動失敗または異常終了を検知する。 | 検証失敗として扱い、必要に応じてトレースログへ記録する。 | 回答表示を保留する。 | 設定上限内でシステムが再生成する。 |
| 検証出力欠落 | 完了イベントを受信したが検証結果候補を取得できない。 | 検証処理失敗として扱い、必要に応じてトレースログへ記録する。 | 回答表示を保留する。 | 設定上限内でシステムが再生成する。 |
| 検証用会話継続ID不正 | 2回目以降の検証で指定した検証用Codex側の会話継続IDで再開できない。 | 検証処理失敗として扱い、必要に応じてトレースログへ記録する。 | 回答表示を保留する。 | 設定上限内でシステムが再生成する。 |
...
| 項目名 | 必須 | 型・形式 | 制約 | 備考 |
| --- | --- | --- | --- | --- |
| 利用者ID | 必須 | 識別子 | 作業領域分離に利用する。 | MVPでは共有1ユーザを指す。 |
| セッションID | 必須 | 識別子 | 生成用と検証用で同じ値を使う。 | チャット単位の作業領域に対応する。 |
...
| 作業ディレクトリ | 必須 | パス | 利用者IDとセッションIDで分離する。 | OS依存表現は利用者へ表示しない。 |
...
| 未分類エージェントメッセージ | 任意 | 構造化データ | 中間メッセージか最終結果か未確定の最新メッセージ。 | バックエンドが1件だけ保持する。 |

### 6.2. JSONLメッセージ判定

| 判定対象 | 扱い |
| --- | --- |
| エージェントメッセージ受信時 | 直ちに中間メッセージまたは最終結果へ確定せず、未分類エージェントメッセージとして1件だけ保持する。 |
| 後続の処理継続イベント受信時 | 保持中の未分類エージェントメッセージを中間メッセージ候補にする。 |
| 別のエージェントメッセージ受信時 | 直前の未分類エージェントメッセージを中間メッセージ候補にし、新しいメッセージを未分類として保持する。 |
| 完了イベント受信時 | 保持中の未分類エージェントメッセージを最終回答候補または検証結果候補にする。 |
| 失敗、キャンセル、タイムアウト、プロセス異常終了時 | 保持中の未分類エージェントメッセージを破棄し、最終結果として採用しない。 |

中間メッセージ候補は、出力スキーマに適合し、表示本文を安全に抽出できる場合だけ利用者画面へ配信する。抽出できない場合は、バックエンドが処理段階に応じた定型メッセージを配信する。
...
- 生成用と検証用は同じ利用者IDとセッションIDを使って対応付けるが、生成用Codex側の会話継続IDと検証用Codex側の会話継続IDは別々に管理する。
- 共有データソースはセッションごとに複製しない。
- 中間メッセージ候補やJSONLイベントには内部パス、秘密情報、環境変数を表示しない。
- コマンド実行時の通知に含まれるコマンド文字列、標準出力、絶対パスは利用者画面へ直接表示しない。
- Windows/Linuxの具体的なプロセス制御差異はバックエンド内部に閉じ込める。
