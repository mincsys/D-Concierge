...
| `ui.sub_welcome_message` | 任意 | 開始画面のウェルカムメッセージ下に表示する補足文言。YAMLの `\n` による改行指定を表示上の改行として扱う。 | `GET /api/app-config` |
...
| `trace_log.retention_days` | 必須 | トレースログの日付ディレクトリを保持する日数。正の整数を指定する。標準は90日。 | ログ設計 |
| `trace_log.max_files_per_day` | 必須 | アプリケーション起動ごとの同日トレースログ最大保存件数。正の整数を指定する。標準は1000件。 | ログ設計 |
...
| `sub_welcome_message` | `ui.sub_welcome_message` | 表示しない。 |
...
- Codex出力、DB保存値、API/SSE公開値として扱うファイル参照は、バックエンド内部でPOSIX相対形式へ標準化する。Windows形式の絶対パス、UNC、URL、親ディレクトリ参照は参照先として解釈せず拒否する。
...
| UI設定不足 | ウェルカムメッセージ、補足案内文、入力候補チップのうち未設定の項目を表示しない。 |
