...
- 回答生成と回答検証はDockerコンテナ内のCodex実行で実現する。
...
flowchart LR
    ...
    source["共有データソース"]
    workdir["Codex作業領域"]
    artifacts["Codex成果物保存領域"]
    trace["トレースログ"]

    browser -->|画面表示| frontend
    frontend -->|REST・SSE| backend
    backend -->|履歴・回答保存| db
    backend -->|回答生成依頼| gen
    backend -->|回答検証依頼| val
    backend -->|参照元配信| source
    backend -->|作成・削除| workdir
    backend -->|保存・配信| artifacts
    backend -->|障害調査記録| trace
    gen -->|読取| source
    gen -->|読み書き| workdir
    val -->|読取| source
    val -->|読み書き| workdir
...
| Codex実行基盤 | Docker | 生成用・検証用Codex実行コンテナを実行ごとに起動する。 |
...
| キャンセル | フロントエンド、バックエンド、Codex実行制御 |
| トレースログ保存 | バックエンド |

## 6. Windows/Linux両対応方針

- 利用者操作、画面表示、API契約、SSE契約はOS差異を持たない。
- Dockerコンテナの起動、キャンセル、パス正規化はバックエンド内部の実行制御で吸収する。
