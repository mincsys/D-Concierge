...
`raw/meta/*.json` の概要・目次と `html/*/index.html` のページ別 HTML から、ユーザが探している文章が記載された文書と PDF ページ番号を特定する。

想定データ構造:

- `raw/meta/<文書名>.json`: `title`、`summary`、`table_of_contents` を持つメタデータ。
- `html/<文書名>/index.html`: `<section class="page" id="page-N">` で PDF ページごとに区切られた HTML。
- meta と HTML は `<文書名>` のベース名一致で対応させる。

## 基本ワークフロー

1. `raw/meta/*.json` をすべて読み、各文書の `title`、`summary`、`table_of_contents` を把握する。
...
```powershell
uv run python .\.codex\skills\doc-html-finder\scripts\search_toc_pages.py ".\html\<文書名>\index.html" "安全要求"
```
...
```powershell
uv run python .\.codex\skills\doc-html-finder\scripts\search_toc_pages.py ".\html\<文書名>" "安全要求" --terms "合意" "セキュリティ"
```

### ページ section を抽出する

```powershell
uv run python .\.codex\skills\doc-html-finder\scripts\extract_html_pages.py ".\html\<文書名>\index.html" 34,35,36
```
...
```powershell
uv run python .\.codex\skills\doc-html-finder\scripts\extract_html_pages.py ".\html\<文書名>\index.html" 34 --context 1
```
