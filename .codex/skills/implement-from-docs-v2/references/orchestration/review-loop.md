...
3. 機能別総合テスト完了時
   ...
   - 検証役は、生成役が対象ケースの前提条件、操作手順、期待結果を仕様書どおりに実施したか確認する。API確認、DB確認、結合テスト、ログ確認などが補助証跡を超えて代替合格根拠になっている場合は指摘する。
   - 仕様どおりに実施できなかったケースは、未実施の手順番号、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件、保留妥当性を確認する。
   - 実装不足、設定不足、テストデータ不足、手順誤り、証跡不足、実施可能なのに未実施の項目は保留にせず issue 化する。
   - 後続機能、外部認証、自動操作不可、環境制約など、生成役が修正しても現時点で解消できない場合だけ、正式総合テストへの持ち越しを許可する。
...
3. 指摘、実行結果不足、証跡不足、判断不能がある場合、`.issue/implement-from-docs/` に 1 指摘 1 ファイルで保存する。機能別総合テストレビューでは、仕様どおりに実施できなかったケースのうち修正可能な問題を issue 化する。正式総合テスト結果分析レビューでは、不合格・保留ケースの原因分類と推奨修正方針を issue に含める。
...
機能別総合テストの修正ループでは、管理役は当該機能の `state.md` を更新する。仕様どおりに実施できなかったケースは `仕様どおり実施できなかったケース` に記録し、主業務パスの中核ケースが未確認の場合は `中核未確認` に記録する。正式総合テストへの持ち越しは、検証役が保留妥当と判定した項目だけを当該機能の `正式総合テストへの持ち越し` に記録する。
...
- `部分確認` は、未実施の手順番号、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件が記録されている場合だけ許容候補にする。
- `部分確認`、`後続機能待ち`、`外部認証待ち`、`自動操作不可`、`環境制約`、`対象外` は、検証役が保留妥当と判定し、正式総合テストへの持ち越しが state に記録されている場合に限り、機能結合完了へ進める。
- 実装不足、設定不足、テストデータ不足、手順誤り、証跡不足は保留にせず修正可能 issue として扱う。
- 主業務パスの中核ケースが未確認の場合は、機能結合完了後も最終完了扱いにしない。
