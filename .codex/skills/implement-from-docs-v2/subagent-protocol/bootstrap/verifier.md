...
- 機能別総合テストレビューでは、総合テスト仕様の `前提条件`、`操作手順`、`期待結果` どおりに実施されているか、補助証跡が代替合格根拠になっていないか、保留妥当性があるかを確認する方針を理解する。
- 仕様どおり実施できない項目のうち、実装不足、設定不足、テストデータ不足、手順誤り、証跡不足は保留にせず issue 化し、後続機能待ち、外部認証待ち、自動操作不可、環境制約、対象外だけを正式総合テストへの持ち越し候補にできる方針を理解する。
