...
あなたはこの依頼で、対象機能の機能別総合テスト結果、証跡、分類、`docs` 非変更、仕様どおりの実施可否、保留妥当性、正式総合テストへの持ち越しをゼロベースでレビューし、機能結合完了可否を判定します。

## 今回の目的

対象機能を機能結合完了扱いしてよいか、機能別総合テストが総合テスト仕様の前提条件、操作手順、期待結果どおりに実施されているか、仕様どおりに実施できない項目の保留理由が妥当か、正式総合テストへの持ち越しが許容できるかの観点から判定します。
...
- 対応総合テスト仕様と主対象ケース ID を確認し、生成役が対象ケースの `前提条件`、`操作手順`、`期待結果` を改変せず実施したか確認する。
- API確認、DB確認、結合テスト、ログ確認などが補助証跡として扱われ、総合テストの代替合格根拠として扱われていないか確認する。
- `合格 / 不合格 / 部分確認 / 後続機能待ち / 外部認証待ち / 自動操作不可 / 環境制約 / 対象外` の分類が妥当か確認する。
- Sandbox 承認が必要なだけの項目が、承認付き実行なしに未実施または `環境制約` へ分類されていないか確認する。
- 仕様どおりに実施できなかった項目について、未実施の手順番号、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件が記録されているか確認する。
- 仕様どおりに実施できなかった項目を `修正可能 / 後続機能待ち / 外部認証待ち / 自動操作不可 / 環境制約 / 対象外` に分類する。
- 実装不足、設定不足、テストデータ不足、手順誤り、証跡不足、実施可能なのに未実施の項目、未分類の未実施がないか確認する。該当する場合は保留にせず issue 化する。
- 主業務パスの中核ケースが未確認の場合、`中核未確認` として報告し、機能結合完了可否と最終完了扱い可否へ反映する。
...
- `部分確認`、`後続機能待ち`、`外部認証待ち`、`自動操作不可`、`環境制約`、`対象外` は、分類理由が妥当で正式総合テストへの持ち越しが明確な場合だけ許容する。Sandbox 承認が必要なだけで承認付き実行を試していない `環境制約` は許容しない。
- 実装不足、設定不足、テストデータ不足、手順誤り、証跡不足は、保留妥当ではなく修正可能 issue として扱う。
- 後続機能、外部認証、自動操作不可、環境制約など、生成役が修正しても現時点で解消できない場合だけ、正式総合テストへの持ち越しを許可できる。
- 主業務パスの中核ケースが未確認の場合、持ち越し妥当でも state に `中核未確認` を記録するよう報告し、最終完了扱い不可と判定する。
