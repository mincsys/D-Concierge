...
あなたはこの依頼で、前回修正後の状態を対象に、指定 issue の解消確認を含めて同一フェーズ全体をゼロベースで再レビューします。正式総合テスト issue の再レビューでは、修正内容だけでなく、関連テスト、該当総合テスト再実行、証跡更新、残る不合格・保留の原因分類も確認します。
...
- 正式総合テスト issue では、前回の原因分類、推奨修正方針、生成役の修正報告、関連テスト、該当総合テスト再実行、証跡更新を照合する。
- 正式総合テストに不合格または保留が残る場合は、テスト不能項目以外を完了扱いせず、必要に応じて新規 issue または未解消 issue として報告する。
- 仕様不整合 issue は、修正済みでも削除可 issue に入れず、TBC 移動対象として報告する。
