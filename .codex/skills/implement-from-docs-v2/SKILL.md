...
description: Implement software from docs by having a manager decompose work into business-feature tasks that are easy to verify with integration and system tests, reuse generator and verifier subagents, create unit and integration tests before implementation, complete strict feature-level system-test review for every feature using .tmp copies of the official system-test specs/evidence, run cross-review before final official system tests, then run official system tests against docs and loop on verifier-created issues for fixable failures or holds under .issue/implement-from-docs.
---
...
docs を正として、管理役、生成役、検証役の 3 役で実装、テスト、検証を進める。v1 の docs-first、TDD、単体・結合・総合テスト、品質ゲートを維持しつつ、v2 では結合テストと総合テストを通しやすい検証可能な業務機能単位で開発し、機能別総合テストまで細かく回し、全機能の機能結合完了後、正式総合テストを実施する。

管理役は業務機能単位への分解、総合テストケース ID の対応付け、サブエージェントへの依頼、状態更新だけを担う。生成役はテスト方針書と設計書から単体・結合テストコードを先行作成し、機能別には実装、結合テスト、`.tmp` 上の機能別総合テストまでを行う。機能別総合テストでは正式総合テストと同じ仕様書、記録形式、evidence 保存方針を使うが、`docs/04_テスト/04_総合テスト/` は変更せず、`.tmp/implement-from-docs-v2/features/<機能>/system-test/` にコピーした作業用ファイルへ結果と証跡を保存する。生成役は対象ケースの前提条件、操作手順、期待結果どおりに実施し、API 確認、DB 確認、結合テスト、ログ確認などを自分の判断で総合テストの代替合格根拠にしない。仕様どおりに実施できない場合は、未実施の手順番号、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件を記録する。検証役だけが保留妥当性を判断し、実装不足、設定不足、テストデータ不足、手順誤り、証跡不足は issue 化する。全機能の機能結合完了後、検証役が横断レビューを行い、指摘がある場合は正式総合テストへ進む前に修正ループを回す。横断レビュー通過後、生成役が `docs/04_テスト/04_総合テスト/` を初めて更新する正式総合テストを行う。検証役は各レビュー時に `review-artifacts` のチェックリストを `.tmp` の対象フェーズへコピーして確認し、テストコード、結合テスト、機能別総合テスト、実装品質、正式総合テスト、完了可否を判定する。正式総合テストで不合格または保留が残る場合、検証役は単なる差し戻しではなく原因分析と issue 作成を行い、管理役は修正可能な issue を生成役へ修正させる。
...
5. [feature-decomposition.md](references/core/feature-decomposition.md) に従い、対象を結合テストと総合テストを通しやすい検証可能な業務機能単位へ分解し、各機能に対応する総合テスト仕様ファイル、主対象ケース ID、後続依存ケース ID を整理する。
...
- Sandbox 環境で承認が必要なコマンドは、承認が必要であることだけを理由に保留、未実施、`環境制約` に分類しない。実行担当者は承認付きで実行し、ユーザが承認しない場合、または承認付き実行後も環境制約で実行不能な場合だけ保留分類にする。
- docs から一意に分かることをユーザへ質問しない。探索で解決できない仕様判断だけ質問する。
- 管理役だけが `.tmp/implement-from-docs-v2/tasklist/implementation-tasklist.md` を編集する。
- 管理役は機能分解と進行管理に集中し、編集候補ファイル一覧を作らない。ただし、機能別総合テストで仕様書どおりに確認する対象を明確にするため、対応する総合テスト仕様ファイル、主対象ケース ID、後続依存ケース ID は整理して state と依頼に含める。
- 管理役は、機能分解時に総合テスト方針と `テスト仕様・結果/*.md` を読み、依存関係だけでなく総合テストケースを仕様書どおりに実施できる最短経路を優先して順序を決める。
...
- 生成役は機能別総合テストで、管理役が指定した対象ケースの `前提条件`、`操作手順`、`期待結果` どおりに実施する。API 確認、DB 確認、結合テスト、ログ確認などを、自分の判断で総合テストの代替合格根拠にしない。
- 生成役は機能別総合テストを仕様どおりに実施できない場合、未実施の手順番号、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件を記録する。補助確認は `補助証跡` として扱い、合格扱いできるかは検証役の判定に委ねる。
...
- 検証役は機能別総合テストで、生成役が総合テスト仕様の操作手順を改変していないか、仕様どおりに実施できなかった項目の理由が妥当かを確認する。実装不足、設定不足、テストデータ不足、手順誤り、証跡不足は保留にせず issue 化する。
- 検証役は機能別総合テストで仕様どおりに実施できなかった項目を `修正可能 / 後続機能待ち / 外部認証待ち / 自動操作不可 / 環境制約 / 対象外` に分類する。後続機能、外部認証、自動操作不可、環境制約など、生成役が修正しても現時点で解消できない場合だけ、正式総合テストへの持ち越しを許可する。
- 主業務パスの中核ケースが未確認の場合、持ち越し妥当でも state に `中核未確認` と記録し、当該機能やアプリを最終完了扱いしない。
...
- ユーザが確定した対象範囲が、結合テストと総合テストを通しやすい検証可能な業務機能単位へ分解されている。
- 各機能について、対応する総合テスト仕様ファイル、主対象ケース ID、後続依存ケース ID が整理されている。
...
- 機能別総合テストでは、対象ケースの前提条件、操作手順、期待結果どおりの実施可否が記録され、仕様どおりに実施できなかった項目は未実施手順、阻害要因、確認済み期待結果、未確認期待結果、補助証跡、再実施条件へ分解されている。
- 検証役が、機能別総合テストの保留妥当性、主業務パスの中核未確認、修正可能 issue の有無を判定している。
