...
レビューでは、使用する checklist を作業用 checklist 保存先へコピーし、コピーした checklist に各項目の検証結果と根拠を記録する。作業用 checklist 保存先は、ユーザまたは呼び出し元スキルが明示した場合はその場所を使い、指定がない場合は `.tmp/review/<YYYY-MM-DD_HH-MM-SS>_<レビュー対象名>/` を使う。レビューで見つけた指摘は、チャット本文ではなく指摘保存先へ 1 指摘 1 ファイルで保存する。指摘保存先は、ユーザまたは呼び出し元スキルが明示した場合はその場所を使い、指定がない場合は `.issue/review/` を使う。レビュー実行中に対象成果物を修正しない。
...
4. 作業用 checklist 保存先を決める。ユーザまたは呼び出し元スキルが `checklist保存先` または `作業用 checklist 保存先` を明示した場合はその場所を使い、指定がない場合は `.tmp/review/<YYYY-MM-DD_HH-MM-SS>_<レビュー対象名>/` を使う。
5. [reading-order.md](references/reading-order.md) に従い、`AGENTS.md`、レビュー対象、関連成果物、対応する作成元スキル、使用する checklist、[checklist-record-format.md](references/checklist-record-format.md)、実際の指摘保存先にある既存指摘を読む。
6. 使用する checklist を作業用 checklist 保存先へコピーする。作成元スキルの `references/review-checklist.md` を主チェックリストとして使う場合も、原本ではなく作業用 checklist へコピーする。
7. レビュー対象と関連成果物を確認し、仕様、設計、実装、テスト、証跡の乖離や品質問題を洗い出す。
8. 作業用 checklist の各項目をカテゴリ単位で検証し、[checklist-record-format.md](references/checklist-record-format.md) に従って `- [x]`、`検証結果`、`確認根拠`、必要な `指摘`、`理由`、`不足根拠` を項目直下へ記録する。`- [x]` は合格ではなく、証拠付きで検証処理が完了したことを表す。
9. 指摘が必要な場合は [finding-file-format.md](references/finding-file-format.md) に従い、指摘保存先へ 1 指摘 1 ファイルで保存する。同じ問題が既存 issue にある場合は、重複作成せず既存 issue パスを使う。
10. 最終応答では、作業用 checklist 保存先、作成した指摘ファイルと既存指摘の件数およびパスだけを報告する。指摘がない場合は、作業用 checklist 保存先と `レビュー指摘はありませんでした。` を返す。他スキルから呼び出され、報告形式が明示された場合は、呼び出し元が求める判定項目も返してよい。

## レビュー種別

- `documentation`: [documentation-review-checklist.md](references/documentation-review-checklist.md) を使う。対応する作成元スキルの `references/review-checklist.md` がある場合は主チェックリストとして扱い、作業用 checklist 保存先へコピーする。原本は変更しない。
...
- `cross-artifact`: [cross-artifact-review-checklist.md](references/cross-artifact-review-checklist.md) を必ず使い、docs、src、tests、evidence、設定、実行結果の追跡と整合を照合する。対象に応じて `documentation`、`implementation`、`test`、`evidence` の checklist も合わせて作業用 checklist 保存先へコピーする。
...
- 詳細なレビュー本文をチャットへ貼らない。
- checklist 原本は変更しない。更新してよいのは作業用 checklist 保存先へコピーした checklist だけにする。
- 作業用 checklist は [checklist-record-format.md](references/checklist-record-format.md) に従って更新する。
- `- [x]` は合格ではなく、証拠付きで検証処理が完了したことを表す。結果は項目直下の `検証結果` に記録する。
- 作業用 checklist の `検証結果: 指摘あり` は、指摘ファイルまたは既存指摘のパスと対応させる。
...
- 使用したレビュー種別の checklist を作業用 checklist 保存先へコピーしている。
- checklist 原本を更新していない。
- 作業用 checklist の各項目に `- [x]`、`検証結果`、`確認根拠` が記録され、未処理の `- [ ]` が残っていない。
- `検証結果: 指摘あり` の項目が、作成済みまたは既存の指摘ファイルパスと対応している。
- `検証結果: 対象外` の項目に `理由` が記録されている。
- `検証結果: 判断不能` の項目に `不足根拠` が記録され、最終応答または呼び出し元が求める判定項目に反映されている。
...
- 最終応答が、作業用 checklist 保存先、作成した指摘ファイルと既存指摘の件数およびパスだけになっている。他スキルから呼び出され、報告形式が明示された場合は、呼び出し元が求める判定項目を含んでよい。
- 指摘がない場合、最終応答が作業用 checklist 保存先と `レビュー指摘はありませんでした。` だけになっている。他スキルから呼び出され、報告形式が明示された場合は、呼び出し元が求める判定項目を含んでよい。
...
- checklist 記録形式: [checklist-record-format.md](references/checklist-record-format.md)
